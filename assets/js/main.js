$(function () {
	...
	$("#create_eventform").submit(function(e){
		var data = new FormData(this);
		var url = location.protocol+'//'+location.host+'/admin/saveEvent';
		$.ajax({
			method:'post',
			...
			cache: false,
			processData: false,
			url: location.protocol+'//'+location.host+'/admin/saveEvent',
			success: function(data){
				if(data.result){
					$("#modal-config").find('.modal-title').html('<p class="alert alert-success">Success</p>');
					$("#modal-config").find('.modal-body').html(data.msg);
					$("#modal-config").find('.modal-footer').html('');
					$("#modal-config").modal();
					setTimeout(function(){
						url = location.protocol+'//'+location.host+'/admin/event/create/'+data.data;
